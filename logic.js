...
function slideLeft(){
	for(let r=0; r<rows; r++){
        ...
        // This is line for animation
        // copy the content of original row
		...
		for(let c=0; c<columns; c++){
            ...
            if(originalRow[c] !== num && num !== 0){
                ...
                // remove the animation class after animation is finished
                setTimeout(() => {
...
//  event listener to check where you touch the screen and prevents scrolling. input targets any element that includes word tile
...
// listen for the touch end evvent on the entire document.
document.addEventListener("touchend", (e) => {

    // check if the element triggered has a class name "tile"
    if (!e.target.className.includes("tile")) {
        return; // exit the function (listener won't work)
    }
